...
/** See `quickadui-config.ts`'s `QuickaduiConfigResourceEndpointAction` (same shape, this is the codegen-facing mirror of it). */
export interface ResourceEndpointAction {
  readonly path?: string;
  readonly method?: ResourceEndpointMethod;
}

/** See `quickadui-config.ts`'s `QuickaduiConfigResourceListEndpoint` (same shape, codegen-facing mirror). */
export interface ResourceListEndpoint extends ResourceEndpointAction {
  readonly responseShape?: "wrapped" | "array";
}

/** See `quickadui-config.ts`'s `QuickaduiConfigResourceEndpoints` (same shape, codegen-facing mirror). */
export interface ResourceEndpoints {
  readonly list?: ResourceListEndpoint;
  readonly get?: ResourceEndpointAction;
  readonly create?: ResourceEndpointAction;
  readonly update?: ResourceEndpointAction;
  readonly delete?: ResourceEndpointAction;
}
...
export interface ResourceTemplateOptions {
  ...
  /** Omit entirely for today's default: DummyJSON-style conventions for every action — see `resolveEndpoints`. */
...
interface ResolvedEndpointAction {
  /** Already template-literal-ready: a configured `"{id}"` placeholder has been rewritten to `"${id}"`, so this can be spliced straight into a backtick template. */
  readonly path: string;
  readonly method: ResourceEndpointMethod;
}

interface ResolvedEndpoints {
  readonly list: ResolvedEndpointAction & { readonly responseShape: "wrapped" | "array" };
  readonly get: ResolvedEndpointAction;
  readonly create: ResolvedEndpointAction;
  readonly update: ResolvedEndpointAction;
  readonly delete: ResolvedEndpointAction;
}

/**
 * Fills in every action's default path/method/responseShape from
 * `endpoint`, then overlays whatever `overrides` sets — the single
 * source of truth `renderResourceApi`/`renderResourceListPage` both read
 * from, so they can never disagree about a path/method/shape. A `{id}`
...
function resolveEndpoints(
  ...
  const del = overrides?.delete;
  return {
    list: {
      path: toTemplate(list?.path ?? endpoint),
...
export function renderResourceApi(options: ResourceTemplateOptions): ScaffoldFile {
  ...
  const endpoints = resolveEndpoints(endpoint, options.endpoints);
  const listIsArray = endpoints.list.responseShape === "array";

  const listResultType = listIsArray
    ? `export type ${plural}ListResult = readonly ${pascal}[];`
  ...
  const listFn = listIsArray
    ? `export async function list${plural}(): Promise<${plural}ListResult> {
  const response = await fetch(\`\${API_BASE}/${endpoints.list.path}\`${methodInit(endpoints.list.method, "GET")});
  if (!response.ok) {
...
export async function list${plural}(options: List${plural}Options = {}): Promise<${plural}ListResult> {
  ...
  const response = await fetch(\`\${API_BASE}/${endpoints.list.path}?limit=\${limit}&skip=\${skip}\`${methodInit(endpoints.list.method, "GET")});
  if (!response.ok) {
    throw new Error(\`Failed to load ${endpoint} (\${response.status}).\`);
  }
  return (await response.json()) as ${plural}ListResult;
...
// you have one. If its REST shape differs from what's below — a
// different path, HTTP method, or list-response shape per action — set
// resources[].endpoints in your quickadui.config.json and regenerate via
// \`quickadui apply\` instead of hand-editing this file; see
...
export async function get${pascal}(id: number): Promise<${pascal}> {
  const response = await fetch(\`\${API_BASE}/${endpoints.get.path}\`${methodInit(endpoints.get.method, "GET")});
  if (!response.ok) {
...
export async function create${pascal}(input: ${pascal}Input): Promise<${pascal}> {
  const response = await fetch(\`\${API_BASE}/${endpoints.create.path}\`, {
...
export async function update${pascal}(id: number, input: ${pascal}Input): Promise<${pascal}> {
  const response = await fetch(\`\${API_BASE}/${endpoints.update.path}\`, {
...
export async function delete${pascal}(id: number): Promise<void> {
  const response = await fetch(\`\${API_BASE}/${endpoints.delete.path}\`, { method: "${endpoints.delete.method}" });
  if (!response.ok) {
...
export function renderResourceListPage(options: ResourceTemplateOptions): ScaffoldFile {
  ...
  // The two response shapes need genuinely different state/effect code,
  // not just a different `list${plural}` call: "wrapped" gets its
  // current page and its total straight from the server on every `skip`
  // change (today's original behavior), while "array" has no server-side
  // pagination to ask for, so it fetches the full list once and slices
  // it locally for display — see `resolveEndpoints`'s own doc comment.
  // Both variants end by defining the same `pagedItems`/`total`/
  // `totalPages`/`currentPage`, so the JSX below never has to branch.
  const dataSectionJs = listIsArray
...
${handleDeleteBody}
  ...
  // This backend's list endpoint returns a plain array with no
  // server-side total/skip/limit (see resources[].endpoints.list in
  // quickadui.config.json) — paginate the already-fetched list locally.
  const total = items.length;
  const pagedItems = items.slice(skip, skip + PAGE_SIZE);
