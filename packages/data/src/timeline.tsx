...
export function TimelineItem({ className, ...props }: ComponentProps<"li">) {
  return <li data-slot="timeline-item" className={cn("relative flex gap-4 pb-8 last:pb-0", className)} {...props} />;
}
...
export const timelineDotVariants = cva("z-10 size-3 shrink-0 rounded-full border-2 border-neutral-1 ring-2", {
  variants: {
    variant: {
...
export interface TimelineDotProps extends ComponentProps<"span">, VariantProps<typeof timelineDotVariants> {}

export function TimelineDot({ className, variant, ...props }: TimelineDotProps) {
  return <span data-slot="timeline-dot" className={cn(timelineDotVariants({ variant }), className)} {...props} />;
}

export function TimelineConnector({ className, ...props }: ComponentProps<"div">) {
  return <div data-slot="timeline-connector" className={cn("w-px flex-1 bg-neutral-6", className)} {...props} />;
}
