 ...
 */
export function renderAddNextSteps(packages: readonly QuickaduiPackageDefinition[], packageManager: PackageManager): string {
  const installCommand = packageManager === "yarn" ? "yarn" : `${packageManager} install`;
  const sourceLines = packages.filter((pkg) => pkg.needsSourceDirective).map((pkg) => `@source "./node_modules/${pkg.name}";`);
  ...
  if (sourceLines.length > 0) {
    lines.push("  2. Add the following line(s) to your CSS entry point (after your `@import \"tailwindcss\";`):");
    lines.push("");
    ...
    lines.push("  2. No `@source` lines needed — none of the installed packages hardcode Tailwind classes.");
  }
